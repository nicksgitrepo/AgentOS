...
function schedulerCandidateIdentity(worktreePath) {
  ...
  return {commit: "PRELIMINARY_DIAGNOSTIC", tree: controllerDigest({commit, tree, status, diff_sha256: crypto.createHash("sha256").update(diff).digest("hex"), untracked}), clean: false};
}
...
export function applyGovernanceEvidenceRepair({worktreePath}) {
  ...
  worker = replaceOnce(worker, "const evidence = (gate) => Object.fromEntries(gate.evidence_requirements.map((key) => [key, `${gate.gate_id}:${key}`]));", "const gateEvidence = collectGovernanceGateEvidence({worktreePath, tree: decisionTree, schedulerRoot: process.env.AGENTOS_SCHEDULER_ROOT});\n  const evidence = (gate) => Object.fromEntries(gate.evidence_requirements.map((key) => [key, gateEvidence[gate.gate_id + \":\" + key]]));", "placeholder evidence branch");
