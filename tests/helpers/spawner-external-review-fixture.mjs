...
export function provisionTestExternalSpawnerReview({candidate, install = true}) {
  ...
  if (install) { const sealedAuthority = getSealedCanonicalAuthority(); const reviewProvisioning = prepareProtectedSpawnerReviewProvisioning({sealedAuthority, reviewStoreRoot: root}); installExternalSpawnerReviewStore({sealedAuthority, reviewProvisioning}); }
  return {root, receipt};
}
