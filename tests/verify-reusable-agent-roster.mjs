...
for (const entry of roster.entries) {
  ...
  for (const fixture of entry.hostile_fixtures.fixtures) {
    ...
    // A package that embeds raw registry digests in its executable fixture
    // evidence would form a recursive fixture↔roster hash cycle.  Such a
    // package must declare STATIC_SEMANTIC_ONLY and is checked through the
    // authority binder's semantic/raw provenance mutation proof instead of a
    // self-referential per-fixture file hash.
