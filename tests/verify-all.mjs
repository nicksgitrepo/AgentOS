...
assert.equal(controllerContract.authority_compatibility.canonical_target, "INTENT_REGULATOR");
assert.equal(controllerContract.authority_compatibility.never_target, "CONTROLLER");
...
assert.deepEqual(kernel.permanent_role_authority.canonical_roles, ["AGENT_SPAWNER_COMPILER", "CONTROLLER", "INTENT_REGULATOR", "RUNTIME", "SCHEDULER"]);
assert.equal(kernel.permanent_role_authority.controller, "control/permanent-role-authority.mjs");
assert.equal(kernel.permanent_role_authority.effects.includes("does not start roles"), true);
...
assert.equal(kernel.agentos_controller.authority_status, "LEGACY_COMPATIBILITY_INPUT_ONLY");
