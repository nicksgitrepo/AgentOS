...
const NOW = "2026-08-22T15:41:22.000Z";
...
hostile(({authorityRoot}) => fs.unlinkSync(path.join(authorityRoot, "fixtures/model-policy-evidence/current-host.2026-08-22.json")), /ENOENT|artifact/iu);
hostile(({authorityRoot, snapshot}) => rebindArtifact(authorityRoot, snapshot, "HOST.CODEX_MODEL_CATALOG.2026-08-22", (artifact) => { artifact.observed_at_utc = "2026-08-22T16:00:00.000Z"; }), /future-dated/iu);
...
hostile(({authorityRoot, snapshot}) => {
  rebindArtifact(authorityRoot, snapshot, "HOST.CODEX_MODEL_CATALOG.2026-08-22", (artifact) => artifact.summary.models.forEach((model) => { model.available = false; }));
