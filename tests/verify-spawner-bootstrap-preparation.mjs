...
assert.equal(admission.admission.spawnable, true); assert.equal(admission.admission.worker_spawned, false); assert.equal(admission.hostile_fixture_refs.length, 33); assert(admission.evidence_refs.some((entry) => entry.evidence_id === "EVIDENCE.SPAWNER.HOSTILE_EXECUTION"));
