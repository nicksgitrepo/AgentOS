...
const admission = compileAgentSpawnerGovernedAdmission({adapterId: "ADAPTER.SPAWNER.EXTERNAL.BOOTSTRAP", sourceContinuation: continuation, lifecycleBefore: after, clearanceReceiptSha256: clearance.receiptSha256});
assert.equal(admission.admission.spawnable, true); assert.equal(admission.admission.worker_spawned, false); assert.equal(admission.hostile_fixture_refs.length, 36); assert(admission.evidence_refs.some((entry) => entry.evidence_id === "EVIDENCE.SPAWNER.HOSTILE_EXECUTION"));
assert.throws(() => compileAgentSpawnerGovernedAdmission({adapterId: "ADAPTER.SPAWNER.EXTERNAL.BOOTSTRAP.REPLAY", sourceContinuation: continuation, lifecycleBefore: after, clearanceReceiptSha256: clearance.receiptSha256}), /already consumed/iu);
