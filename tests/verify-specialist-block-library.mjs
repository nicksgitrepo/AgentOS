...
assert.equal(first.records.length, 91, "foundation, reusable standard, P0, P1, and P2 package count is wrong");
...
assert.equal(taskCompilerCatalog.length, 91, "task-shaped compiler catalog must load every compiled candidate package");
assert.equal(taskCompilerCatalog.filter((block) => block.role_kind === "STANDARD_BLOCK").length, 15);
assert.deepEqual(taskCompilerCatalog.filter((block) => block.role_kind === "STANDARD_BLOCK").map((block) => block.block_id), ["specialist.standard.aws-iam-current", "specialist.standard.cloudflare-cache-current", "specialist.standard.cloudflare-dns-current", "specialist.standard.nist-ssdf", "specialist.standard.oauth-rfc-9700", "specialist.standard.oidc-core-1-0", "specialist.standard.openapi-3-1-1", "specialist.standard.owasp-api-top10-2023", "specialist.standard.owasp-asvs", "specialist.standard.owasp-top10-2025", "specialist.standard.postgresql-17-rls", "specialist.standard.react-19-2", "specialist.standard.rust-reference", "specialist.standard.slsa", "specialist.standard.typescript-5-9"]);
...
assert.equal(library.roster.blocks.filter((block) => block.role_kind === "STANDARD_BLOCK").length, 15);
assert.equal(library.roster.blocks.filter((block) => block.role_kind === "ROUTER").length, 12);
assert.equal(library.roster.blocks.filter((block) => block.role_kind === "ATOMIC_SPECIALIST").length, 51);
...
console.log("PASS specialist block library: deterministic foundation/standard/P0/P1/P2 compile, inactive roster, reusable standard digests, 12-gate four-valued semantics, atomic routing, hostile fixture coverage, and exact overlay counts");
