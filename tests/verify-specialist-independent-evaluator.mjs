...
assert.equal(receipt.packages_checked, 91);
assert.equal(receipt.gate_files_checked, 91 * 12);
assert.equal(receipt.hostile_fixtures_checked, 91 * 17);
...
console.log("PASS independent specialist evaluator: 91 inactive packages, 1092 gate files, 1547 hostile fixtures, reusable standard digest checks, dependency closure, atomicity denial, self-admission denial, and read-only receipt");
