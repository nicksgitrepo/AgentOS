...
assert(planned.coverage.outputs.some((row) => row.output_id === "PERSISTENT_RUNTIME"));
assert(planned.coverage.outputs.some((row) => row.output_id === "DATA_AND_MIGRATION_POLICY"));
...
const missingRuntimeAnswers = structuredClone(answers);
