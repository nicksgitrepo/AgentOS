...
When the queued campaign has a separate audit-checkpoint wrapper, the approval packet also carries a content-addressed identity mapping. That mapping keeps the Controller candidate as the canonical campaign identity and records the audit candidate, source commit/tree, audit plan, and settled reconciliation. A local checkpoint may be intentionally unpushed and nonterminal; that is valid development evidence but is never release readiness. A missing or mismatched mapping keeps the candidate queued and inactive.

The control-plane receipt retains the full approval, admission, identity binding, audit candidate, audit plan, every complete audit report body, reconciliation, and current-status record. The Controller writes it atomically under the bounded control-plane root, reads it back as JSON, and fails closed on malformed JSON, stale parent identity, missing report bodies, or any scope, intent, identity, or inactive-boundary mismatch.
