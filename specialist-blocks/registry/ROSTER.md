...
Integrator, Release Manager, and Worktree/Custody Manager are accepted from
their exact independent receipts. The current queue item is
`AGENT.ASSURANCE_ENTERPRISE_ROUTER`; its package, gates, hostile vectors, model
route, and local proof are next for one-at-a-time qualification. All other
