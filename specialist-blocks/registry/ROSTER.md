...
Integrator and Release Manager are accepted from their exact independent
receipts. The current queue item is `AGENT.CONTROL_WORKTREE_CUSTODY`; its
package, gates, hostile vectors, model route, and local proof are next for
one-at-a-time qualification. All other
