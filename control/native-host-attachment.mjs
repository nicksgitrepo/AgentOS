...
import {bindRuntimeIdentity, getRuntimeIdentity, isOpaqueReference, opaqueReference, assertOpaqueReference} from "./opaque-reference.mjs";
...
export function validateHostAttachment(attachment) {
  ...
  for (const [value, label] of [[attachment.attachment_id, "attachment_id"], [attachment.project_id, "project_id"], [attachment.environment_id, "environment_id"]]) stable(value, `native host ${label}`);
  assertOpaqueReference(attachment.host_id, "host", "native host host_id");
...
export function compileHostAttachment({attachment_id, host_id, project_id, environment_id, attached_at_utc}) {
  const publicHostId = isOpaqueReference(host_id, "host")
    ? host_id
    : opaqueReference("host", host_id, `${attachment_id}:${project_id}:${environment_id}`);
  const attachment = {
    schema: HOST_ATTACHMENT_SCHEMA,
    version: 1,
    status: "BOUND",
    attachment_id,
    host_id: publicHostId,
    project_id,
    environment_id,
  ...
  const validated = validateHostAttachment(attachment);
  return isOpaqueReference(host_id, "host") ? validated : bindRuntimeIdentity(validated, {host_id});
}

export function bindNativeHost(host, attachment) {
  validateHostAttachment(attachment);
  validateHostAdapter(host);
  const runtime = getRuntimeIdentity(attachment);
  const context = {attachment_id: attachment.attachment_id, host_id: runtime.host_id, project_id: attachment.project_id, environment_id: attachment.environment_id};
  const bound = {};
