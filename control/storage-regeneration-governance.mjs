...
export function compileGeneratedTempCloseout({
  ...
  contained(rootPath, operationRoot, "generated temp root");
  assert(path.resolve(rootPath) !== path.resolve(operationRoot), "generated temp root may not be the shared operation root", "STORAGE_REGENERATION_SHARED_TEMP_FORBIDDEN");
  ...
  const result = {
    ...
    generation,
    operation_root: path.resolve(operationRoot),
...
export function validateGeneratedTempCloseout(receipt) {
  ...
  string(receipt.operation_root, "generated temp operation root");
  string(receipt.root_path, "generated temp root path");
  contained(receipt.root_path, receipt.operation_root, "generated temp root");
  assert(path.resolve(receipt.root_path) !== path.resolve(receipt.operation_root), "generated temp root may not be shared");
...
export function validateCacheIdentity(cache) {
  exactKeys(cache, ["schema", "version", "cache_id", "owner", "last_use_utc", "size_ceiling_bytes", "expiry_utc", "content_identity", "shared", "consumers", "release_receipt_sha256", "observed_at_utc", "cache_sha256"], "cache identity");
...
export function compileStorageDailyInspection({
  ...
  cycles = [], priorInspection = null, prior_inspection = null, protectedBytes = 0, protected_bytes,
  regeneratedBytes = 0, regenerated_bytes, deletionAttempts = 0, deletion_attempts = 0,
  previousCleanupCycle = null, previous_cleanup_cycle = null,
} = {}) {
  ...
  const prior = prior_inspection ?? priorInspection;
  ...
  if (previous_cleanup_cycle !== null || previousCleanupCycle !== null) validateStorageRegenerationCycle(previous_cleanup_cycle ?? previousCleanupCycle);
  ...
  const result = {
    ...
    previous_cleanup_cycle_sha256: (previous_cleanup_cycle ?? previousCleanupCycle)?.cycle_sha256 ?? null,
