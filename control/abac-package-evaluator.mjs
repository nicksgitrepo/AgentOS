...
import {evaluateAbacBoundary, ABAC_ROUTER_INPUT_SCHEMA} from "./abac-boundary-gate.mjs";

export const ABAC_ROUTER_EVALUATION_SCHEMA = "agentos.specialist_abac_router_package_operational_evaluation.v1";
const ROOT = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..");
const PACKAGE = "specialist-blocks/wave-03/abac";
const BLOCK_ID = "specialist.security.abac";
...
function fail(message, code = "ABAC_ROUTER_EVALUATION_INVALID") { const error = new Error(message); error.code = code; throw error; }
...
function fixtures(root) {
  ...
  assert(names.length === 17 && new Set(names).size === 17, "ABAC fixture inventory invalid", "ABAC_ROUTER_FIXTURE_INVENTORY_INVALID");
  const map = new Map();
  for (const name of names) {
    const file = path.join(root, "fixtures", name); const fixture = json(file);
    assert(fixture.block_id === BLOCK_ID && CLASSES.includes(fixture.class) && fixture.vector?.entrypoint === "control/abac-boundary-gate.mjs#evaluateAbacBoundary", `ABAC fixture is not operational: ${name}`, "ABAC_ROUTER_FIXTURE_UNBOUND");
    assert(fixture.vector.input?.schema === ABAC_ROUTER_INPUT_SCHEMA && fixture.vector.expected_readback?.disposition && fixture.vector.expected_readback?.route && fixture.vector.expected_readback?.error_code, `ABAC fixture lacks typed input/readback: ${name}`, "ABAC_ROUTER_FIXTURE_INVALID");
    assert(!map.has(fixture.class), `ABAC duplicate fixture class: ${name}`, "ABAC_ROUTER_FIXTURE_ALIAS");
    map.set(fixture.class, {fixture, file_sha256: sha(fs.readFileSync(file))});
  }
  assert([...map.keys()].sort().join("\0") === CLASSES.slice().sort().join("\0"), "ABAC fixture classes incomplete", "ABAC_ROUTER_FIXTURE_CLASS_INVENTORY_INVALID");
  return map;
}
...
async function mutation(fixture, digest) {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "agentos-abac-mutation-"));
  try {
    ...
    const target = path.join(control, "abac-boundary-gate.mjs");
    let source = read(path.join(ROOT, "control/abac-boundary-gate.mjs"));
    const anchor = 'if (f.unsafe_action) return result("DENY", "NO_ABAC_SIDE_EFFECT", "ABAC_ROUTER_OPERATION_FORBIDDEN", input);';
    assert(source.includes(anchor), "ABAC mutation anchor missing", "ABAC_ROUTER_MUTATION_ANCHOR_MISSING");
    source = source.replace(anchor, 'if (f.unsafe_action) return result("ROUTE", "ABAC_ATOMIC_SPECIALIST_HANDOFF", "MUTATED_UNSAFE_OPERATION_ALLOWED", input, {routing_allowed: true});');
    fs.writeFileSync(target, source, {flag: "wx"});
    const module = await import(`${pathToFileURL(target).href}?mutation=${Date.now()}`);
    const observed = module.evaluateAbacBoundary(inputFor(fixture, digest));
...
export async function evaluateAbacPackage() {
  const root = path.join(ROOT, PACKAGE); const block = json(path.join(root, "block.json"));
  assert(block.block_id === BLOCK_ID && block.lifecycle === "CANDIDATE" && block.activation === "OFF", "ABAC package state invalid", "ABAC_ROUTER_PACKAGE_STATE_INVALID");
  ...
  assert(files.filter((file) => file.startsWith("gates/") && file.endsWith(".gate")).length === 12, "ABAC gates incomplete", "ABAC_ROUTER_GATE_INVENTORY_INVALID");
  ...
  for (const entry of [...map.values()].sort((a, b) => a.fixture.class.localeCompare(b.fixture.class))) {
    const fixture = entry.fixture; const expected = fixture.vector.expected_readback; const actual = evaluateAbacBoundary(inputFor(fixture, block.block_sha256));
    assert(actual.disposition === expected.disposition && actual.route === expected.route && actual.error_code === expected.error_code, `ABAC vector failed: ${fixture.class}`, "ABAC_ROUTER_HOSTILE_RESULT_FAILED");
    assert(Object.values(actual.external_side_effects).every((value) => value === 0), "ABAC side effect observed", "ABAC_ROUTER_SIDE_EFFECT");
