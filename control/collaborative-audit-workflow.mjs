...
export function aggregateSixAuditReports(wave, reports, {spawnerLifecycleAuthority, independentReviewAuthority} = {}) {
  ...
  const expected = new Set(wave.auditors.map((item) => item.auditor_id)), seen = new Set(), issueIds = new Set(), issues = [], reviewReceipts = [];
...
export function recordIssueAudit(wave, {issueId, passed, auditorGroupIds, freshAuditors, auditorDespawnTransitionReceiptRefs, spawnerLifecycleAuthority, independentReviewAuthority} = {}) {
  ...
  return recordAuditRound(wave, {results: [{issue_id: issueId, passed}], auditorGroupIds, freshAuditors, auditorDespawnTransitionReceiptRefs, spawnerLifecycleAuthority, independentReviewAuthority});
}
