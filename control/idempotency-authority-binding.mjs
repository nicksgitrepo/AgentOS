...
export const IDEMPOTENCY_PROVENANCE_BASE_COMMIT = "5321c583f2144f72105a95315cca48a1a469156f";
export const IDEMPOTENCY_PROVENANCE_BASE_TREE = "7bb718c5392348e487aa9fa2eda79f13bc05ebfd";
export const IDEMPOTENCY_ROLLBACK_STRATEGY = "RESTORE_EXACT_SOURCE_TREE";
export const IDEMPOTENCY_ROLLBACK_RECEIPT_SHA256 = "4196a1971e4d49840a7b5b9c0ae0675b31a010fbf235d74477d11b27f7e613cd";
...
export const IDEMPOTENCY_ROSTER_FILE_SHA256 = "139e0cc2ca70c235d7025e8974184df54eb17c4cc20ccf5c94857131b366ccdc";
export const IDEMPOTENCY_CANONICAL_ARTIFACT_SHA256 = Object.freeze({
  ...
  handoff: "11b30c2bfae3da3ac4f50fe9ea6c00dd96857564337bc724cd30c925598ed6b3",
...
function gitText(args, label) {
  try {
    ...
    fail(`${label} cannot be read from the canonical Git repository`, "IDEMPOTENCY_GIT_PROVENANCE_INVALID");
  }
}
function gitBlob(commit, relativePath) {
  try {
    return execFileSync("git", ["show", `${commit}:${relativePath}`], {cwd: ROOT, encoding: null, stdio: ["ignore", "pipe", "pipe"]});
  } catch {
    return null;
  }
}
function packageFiles() {
  const walk = (directory, prefix) => fs.readdirSync(directory, {withFileTypes: true}).sort((left, right) => left.name.localeCompare(right.name)).flatMap((entry) => {
    const relative = `${prefix}/${entry.name}`;
    const absolute = path.join(directory, entry.name);
    if (entry.isDirectory()) return walk(absolute, relative);
    const stat = fs.lstatSync(absolute);
    assert(stat.isFile() && !stat.isSymbolicLink(), `Idempotency package path ${relative} is not a regular file`, "IDEMPOTENCY_PACKAGE_ARTIFACT_INVALID");
    return [relative];
  });
  return walk(PACKAGE, IDEMPOTENCY_PACKAGE_PATH).sort(compareUtf8);
}
function changedPackageFiles(commit) {
  const baseline = gitText(["ls-tree", "-r", "--name-only", commit, "--", IDEMPOTENCY_PACKAGE_PATH], "Idempotency source package tree").split("\n").filter(Boolean);
  const current = packageFiles();
  return [...new Set([...baseline, ...current])].filter((relativePath) => {
    const previous = gitBlob(commit, relativePath);
    const present = fs.readFileSync(path.join(ROOT, relativePath));
    return previous === null || !Buffer.from(previous).equals(present);
  }).sort(compareUtf8);
}
function checkCommittedSourceAndRollback(handoff, blockSha) {
  assert(handoff.source_commit === IDEMPOTENCY_PROVENANCE_BASE_COMMIT, "Idempotency handoff source commit is not the lane base", "IDEMPOTENCY_SOURCE_COMMIT_INVALID");
  assert(handoff.source_tree === IDEMPOTENCY_PROVENANCE_BASE_TREE, "Idempotency handoff source tree is not the lane base", "IDEMPOTENCY_SOURCE_TREE_INVALID");
  assert(gitText(["rev-parse", `${handoff.source_commit}^{commit}`], "Idempotency source commit") === handoff.source_commit, "Idempotency source commit is not an exact Git object", "IDEMPOTENCY_SOURCE_COMMIT_INVALID");
  assert(gitText(["rev-parse", `${handoff.source_commit}^{tree}`], "Idempotency source tree") === handoff.source_tree, "Idempotency source tree readback differs", "IDEMPOTENCY_SOURCE_TREE_INVALID");
  try {
    execFileSync("git", ["merge-base", "--is-ancestor", handoff.source_commit, "HEAD"], {cwd: ROOT, stdio: "ignore"});
  } catch {
    fail("Idempotency source commit is not an ancestor of the candidate lane", "IDEMPOTENCY_SOURCE_ANCESTRY_INVALID");
  }
  const changed = changedPackageFiles(handoff.source_commit);
  assert(JSON.stringify(changed) === JSON.stringify(handoff.changed_paths), "Idempotency handoff changed-path provenance differs from exact bytes", "IDEMPOTENCY_CHANGED_PATH_PROVENANCE_INVALID");
  assert(handoff.proof.includes(`source_commit:${handoff.source_commit}`) && handoff.proof.includes(`source_tree:${handoff.source_tree}`), "Idempotency handoff omits exact source readback", "IDEMPOTENCY_SOURCE_PROVENANCE_INVALID");
  assert(handoff.proof.includes(`rollback_strategy:${IDEMPOTENCY_ROLLBACK_STRATEGY}`), "Idempotency handoff omits rollback strategy", "IDEMPOTENCY_ROLLBACK_PROVENANCE_INVALID");
  assert(handoff.proof.includes(`rollback_receipt_sha256:${IDEMPOTENCY_ROLLBACK_RECEIPT_SHA256}`), "Idempotency handoff omits rollback receipt", "IDEMPOTENCY_ROLLBACK_PROVENANCE_INVALID");
  const rollbackReceipt = canonicalDigest({candidate_digest: blockSha, source_commit: handoff.source_commit, source_tree: handoff.source_tree, strategy: IDEMPOTENCY_ROLLBACK_STRATEGY});
  assert(rollbackReceipt === IDEMPOTENCY_ROLLBACK_RECEIPT_SHA256, "Idempotency rollback receipt is not canonical", "IDEMPOTENCY_ROLLBACK_PROVENANCE_INVALID");
}
...
export function resolveIdempotencyCanonicalAuthority() {
  ...
  checkCommittedSourceAndRollback(handoffArtifact.value, block.block_sha256);
...
export function assertIdempotencyCommittedHandoff({authority = resolveIdempotencyCanonicalAuthority(), evaluation, handoff, evaluationFileSha256, handoffFileSha256} = {}) {
  ...
  checkCommittedSourceAndRollback(handoff, authority.block_sha256);
  return true;
}
