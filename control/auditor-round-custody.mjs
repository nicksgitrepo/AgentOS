...
function validateGate(gate, index, fixtureIds) {
  exact(gate, ["gate_id", "gate_sha256", "fixture_ids", "executed", "observed_status", "negative_assertions", "result_sha256"], `round.execution.gates[${index}]`);
  id(gate.gate_id, `gate[${index}].gate_id`); sha(gate.gate_sha256, `gate[${index}].gate_sha256`); list(gate.fixture_ids, `gate[${index}].fixture_ids`); unique(gate.fixture_ids, `gate[${index}].fixture_ids`); gate.fixture_ids.forEach((fixtureId) => assert(fixtureIds.has(fixtureId), `gate[${index}] references an unexecuted fixture`, "AUDITOR_ROUND_GATE_FIXTURE_UNBOUND"));
  bool(gate.executed, `gate[${index}].executed`); assert(gate.executed === true, `gate[${index}] was not executed`, "AUDITOR_ROUND_GATE_NOT_EXECUTED"); assert(OBSERVED.has(gate.observed_status), `gate[${index}] status invalid`, "AUDITOR_ROUND_GATE_RESULT_INVALID"); list(gate.negative_assertions, `gate[${index}].negative_assertions`); sha(gate.result_sha256, `gate[${index}].result_sha256`); assert(gate.result_sha256 === digestWithout(gate, "result_sha256"), `gate[${index}] result digest differs`, "AUDITOR_ROUND_GATE_DIGEST_MISMATCH");
}

function validateExecution(execution) {
  exact(execution, ["status", "entrypoints_real", "metadata_only", "timeout_or_silence", "fixture_inventory_complete", "gate_inventory_complete", "fixtures", "gates", "side_effects", "execution_sha256"], "round.execution");
  ...
  list(execution.fixtures, "execution.fixtures"); const fixtureIds = new Set(execution.fixtures.map((fixture) => fixture.fixture_id)); unique([...fixtureIds], "execution.fixtures"); execution.fixtures.forEach(validateFixture);
  list(execution.gates, "execution.gates"); const gateIds = execution.gates.map((gate) => gate.gate_id); unique(gateIds, "execution.gates"); execution.gates.forEach((gate, index) => validateGate(gate, index, fixtureIds)); zeroSideEffects(execution.side_effects, "execution.side_effects"); sha(execution.execution_sha256, "execution.execution_sha256"); assert(execution.execution_sha256 === digestWithout(execution, "execution_sha256"), "execution digest differs", "AUDITOR_ROUND_EXECUTION_DIGEST_MISMATCH");
}
...
export function assessAuditorRound(round) {
  try {
    ...
    const ready = ["PASS", "NOT_APPLICABLE_WITH_EVIDENCE"].includes(round.verdict);
    return Object.freeze({schema: AUDITOR_ROUND_RESULT_SCHEMA, version: 1, disposition: round.verdict, integration_allowed: false, independent_clearance: false, ready_for_spawner_review: ready, error_code: ready ? "AUDITOR_ROUND_TYPED_RESULT_READY" : "AUDITOR_ROUND_TYPED_FINDING_OR_BLOCKER", round_sha256: round.round_sha256, result_sha256: canonicalDigest({schema: AUDITOR_ROUND_RESULT_SCHEMA, version: 1, disposition: round.verdict, integration_allowed: false, independent_clearance: false, ready_for_spawner_review: ready, error_code: ready ? "AUDITOR_ROUND_TYPED_RESULT_READY" : "AUDITOR_ROUND_TYPED_FINDING_OR_BLOCKER", round_sha256: round.round_sha256})});
  } catch (error) {
