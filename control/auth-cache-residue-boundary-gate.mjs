...
export const AUTH_CACHE_RESIDUE_INPUT_SCHEMA = "agentos.auth_cache_residue_boundary_input.v1";
export const AUTH_CACHE_RESIDUE_RESULT_SCHEMA = "agentos.auth_cache_residue_boundary_result.v1";
...
function fail(message, code = "AUTH_CACHE_RESIDUE_INPUT_INVALID") { const error = new Error(message); error.code = code; throw error; }
function assert(value, message, code) { if (!value) fail(message, code); }
function exact(value, allowed, name) {
  assert(value && typeof value === "object" && !Array.isArray(value), `${name} must be an object`, "AUTH_CACHE_RESIDUE_SHAPE_INVALID");
  for (const key of Object.keys(value)) assert(allowed.has(key), `${name} has unknown field ${key}`, "AUTH_CACHE_RESIDUE_UNKNOWN_FIELD");
}
function str(value, name, max = 240) { assert(typeof value === "string" && value.length > 0 && value.length <= max, `${name} is invalid`, "AUTH_CACHE_RESIDUE_FIELD_INVALID"); }
function id(value, name) { str(value, name); assert(ID.test(value), `${name} is not canonical`, "AUTH_CACHE_RESIDUE_ID_INVALID"); }
function digest(value, name) { str(value, name, 64); assert(SHA256.test(value) && !/^([0-9a-f])\1{63}$/u.test(value), `${name} is not a real digest`, "AUTH_CACHE_RESIDUE_DIGEST_INVALID"); }
function result(disposition, route, code, input, extra = {}) {
  const base = {
    schema: AUTH_CACHE_RESIDUE_RESULT_SCHEMA, version: 1, disposition, route,
...
function validate(input) {
  ...
  exact(input.evidence, new Set([
    ...
    "candidate_status", "candidate_digest", "auth_cache_residue_signal", "signal_status", "task_status", "context_status", "context_complete",
  ...
  for (const key of [
    "authority_status", "auth_cache_residue_domain", "control_identity", "control_activity", "control_entity", "control_scope", "control_version",
    "policy_status", "source_status", "source_identity", "source_version", "source_effective_date", "source_retrieved_date",
    "candidate_status", "auth_cache_residue_signal", "signal_status", "task_status", "context_status", "requested_action", "model_policy_status",
  ...
  e.requested_tools.forEach((tool) => { str(tool, "requested_tools[]", 80); assert(TOOLS.has(tool), "unsupported tool", "AUTH_CACHE_RESIDUE_TOOL_SCOPE_INVALID"); });
  for (const key of ["context_complete", "project_data_present", "secret_data_present", "authorization_decision_requested", "policy_mutation_requested", "credential_issue_requested"]) assert(typeof e[key] === "boolean", `evidence.${key} must be boolean`, "AUTH_CACHE_RESIDUE_BOOLEAN_INVALID");
  exact(e.adversarial_flags, new Set(FLAGS), "AUTH_CACHE_RESIDUE adversarial flags");
  Object.values(e.adversarial_flags).forEach((value) => assert(typeof value === "boolean", "adversarial flag must be boolean", "AUTH_CACHE_RESIDUE_BOOLEAN_INVALID"));
  assert(scanPersistedRecord(input).safe, "AUTH_CACHE_RESIDUE input contains protected data", "AUTH_CACHE_RESIDUE_PRIVACY_DENIED");
}

export function evaluateAuthCacheResidueBoundary(input) {
  validate(input); const e = input.evidence;
  if (input.request_kind === "NOT_APPLICABLE" || input.request_kind === "UNRELATED_REQUEST") return result("DENY", "NO_AUTH_CACHE_RESIDUE_SCOPE", "AUTH_CACHE_RESIDUE_SCOPE_NOT_APPLICABLE", input);
  if (FORBIDDEN.has(input.request_kind)) return result("DENY", "NO_AUTH_CACHE_RESIDUE_SIDE_EFFECT", "AUTH_CACHE_RESIDUE_OPERATION_FORBIDDEN", input);
  const f = e.adversarial_flags;
  if (f.unrelated_scope) return result("DENY", "NO_AUTH_CACHE_RESIDUE_SCOPE", "AUTH_CACHE_RESIDUE_SCOPE_NOT_APPLICABLE", input);
  if (f.authority_conflict) return result("DENY", "CONTROL_PLANE_ESCALATION", "AUTH_CACHE_RESIDUE_AUTHORITY_CONFLICT", input);
  if (f.missing_context) return result("DENY", "TYPED_CONTEXT_REQUIRED", "AUTH_CACHE_RESIDUE_CONTEXT_INCOMPLETE", input);
  if (f.protected_data || e.project_data_present || e.secret_data_present) return result("DENY", "PRIVACY_BOUNDARY_REQUIRED", "AUTH_CACHE_RESIDUE_PROTECTED_DATA_FORBIDDEN", input);
  if (f.self_acceptance) return result("DENY", "INDEPENDENT_REVIEW_REQUIRED", "AUTH_CACHE_RESIDUE_SELF_ACCEPTANCE_FORBIDDEN", input);
  if (f.scope_expanded || f.broad_claim) return result("DENY", "NARROW_SCOPE_REQUIRED", "AUTH_CACHE_RESIDUE_SCOPE_EXPANSION_FORBIDDEN", input);
  if (f.duplicate_authority) return result("DENY", "SINGLE_AUTHORITY_REQUIRED", "AUTH_CACHE_RESIDUE_DUPLICATE_AUTHORITY", input);
  if (f.unsupported_tool) return result("DENY", "TYPED_TOOL_SCOPE_REQUIRED", "AUTH_CACHE_RESIDUE_TOOL_SCOPE_INVALID", input);
  if (f.cross_provider || f.stale_source) return result("DENY", "SOURCE_REFRESH_REQUIRED", "AUTH_CACHE_RESIDUE_SOURCE_STALE_OR_UNVERIFIED", input);
  if (e.authorization_decision_requested || e.policy_mutation_requested || e.credential_issue_requested) return result("DENY", "ATOMIC_SPECIALIST_REQUIRED", "AUTH_CACHE_RESIDUE_AUTHORIZATION_SIDE_EFFECT_FORBIDDEN", input);
