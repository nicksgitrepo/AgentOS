...
- `controller-supervisor.mjs` and `controller-supervisor-runtime.mjs` — the self-starting observation, bounded-goal, boundary, lease, heartbeat, and repair-routing loop for the persistent Controller. A protected hold or an explicit authorized resume event may be `ACTIVE_PROTECTED_WAIT`/`ACTIVE_EVENT_WAIT`; an unchanged observation after a routed action is instead a durable no-progress RCA and a repairable failure, never an unexplained idle runtime;
