...
export function assertAwsIamPolicyCanonicalEvidence(evidence, authority = resolveAwsIamPolicyCanonicalAuthority()) {
  assert(evidence.candidate_digest === authority.block_sha256 && evidence.candidate_status === "CURRENT_CANDIDATE", "AWS IAM Policy candidate is not the canonical inert candidate", "AWS_IAM_POLICY_CANDIDATE_BINDING_INVALID");
  assert(evidence.authority_status === "CURRENT" && evidence.custody_status === "BOUND" && evidence.custody_owner === "AGENT.PLATFORM_AWS_IAM_POLICY" && evidence.custody_ref === authority.custody_ref, "AWS IAM Policy authority custody is not canonical", "AWS_IAM_POLICY_CUSTODY_BINDING_INVALID");
  assert(evidence.authority_scope === "AWS_IAM_POLICY" && evidence.scope === "NARROW" && evidence.provider_identity === "AWS" && evidence.provider_version === "CURRENT" && evidence.policy_identity === "IAM_POLICY_ELEMENTS", "AWS IAM Policy authority scope is not canonical", "AWS_IAM_POLICY_AUTHORITY_SCOPE_INVALID");
  ...
  assert(evidence.source_status === "CURRENT_VERIFIED" && evidence.source_identity === authority.source_identity && evidence.source_version === authority.source_version && evidence.source_effective_date === authority.source_effective_date && evidence.source_retrieved_date === authority.source_retrieved_date, "AWS IAM Policy source evidence is not current and exact", "AWS_IAM_POLICY_SOURCE_IDENTITY_INVALID");
  assert(evidence.signal === "CLOUD.AWS_IAM" && evidence.signal_status === "BOUND" && evidence.policy_status === "BOUND" && evidence.policy_scope === "IAM_POLICY_ELEMENTS", "AWS IAM Policy provider signal is not canonical", "AWS_IAM_POLICY_CONTEXT_BINDING_INVALID");
  assert(evidence.model_policy_status === authority.model.snapshot_status && evidence.model_snapshot_sha256 === authority.model.snapshot_sha256 && evidence.model_task_class === authority.model.task_class && evidence.model_capability_floor === authority.model.minimum_capability && JSON.stringify(evidence.model_required_capabilities) === JSON.stringify(authority.model.required_capabilities) && evidence.model_route_sha256 === authority.model_route_sha256 && evidence.model_route_status === "BOUND", "AWS IAM Policy model route is not bound to the canonical snapshot", "AWS_IAM_POLICY_MODEL_ROUTE_UNBOUND");
  assert(evidence.context_status === "AWS_IAM_POLICY_CONTEXT" && evidence.context_receipt_sha256 === authority.context_sha256, "AWS IAM Policy typed context receipt is not canonical", "AWS_IAM_POLICY_CONTEXT_RECEIPT_INVALID");
