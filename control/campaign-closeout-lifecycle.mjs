...
// Scheduler projection/liveness is deliberately a read-only, host-neutral
// evidence boundary.  A blank application projection is only one source in a
// triangulation; it can never, by itself, authorize a stall or a replacement.
...
function normalizeDurableSessionObservation(value) {
  ...
  requireIdentifier(sessionId, "durable session ID");
  const ordinal = pick(value, "ordinal", "session_ordinal", "sessionOrdinal");
  nonNegativeInteger(ordinal, "durable session ordinal");
  ...
  const activitySha = SHA256.test(String(activity)) && typeof activity === "string" ? activity : canonicalDigest(activity);
  return {
...
export function compileSchedulerProjectionLivenessObservation({
  ...
  requireIdentifier(taskId, "scheduler liveness task ID");
  requireIdentifier(laneId, "scheduler liveness lane ID");
  ...
  nonNegativeInteger(projectionCount, "scheduler projected item count");
  ...
  nonNegativeInteger(recoveryAttempt, "same-task recovery attempt");
  nonNegativeInteger(maxRecoveryAttempts, "maximum same-task recovery attempts");
...
export function validateSchedulerProjectionLivenessObservation(receipt) {
  ...
  requireIdentifier(receipt.task_id, "scheduler projection/liveness task ID");
  requireIdentifier(receipt.lane_id, "scheduler projection/liveness lane ID");
  ...
  nonNegativeInteger(receipt.projection.items_count, "scheduler projection/liveness app item count");
...
export function deduplicateMaterialLivenessEscalation({escalation, ledger} = {}) {
  ...
  requireIdentifier(escalation.task_id, "material liveness escalation task ID");
  requireIdentifier(escalation.lane_id, "material liveness escalation lane ID");
  requireSha(escalation.evidence_sha256, "material liveness escalation evidence digest");
