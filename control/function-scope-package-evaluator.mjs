...
export async function evaluateFunctionScopePackage() {
  ...
  for (const entry of [...map.values()].sort((a, b) => a.fixture.class.localeCompare(b.fixture.class))) {
    const fixture = entry.fixture; const input = inputFor(fixture.class, block.block_sha256); const actual = evaluateFunctionScopeBoundary(input); const expectedDeny = ["authority_conflict", "broad_when_narrow_exists", "cross_provider_version_claim", "duplicate_sibling_authority", "false_positive", "missing_context", "router_self_accept", "silent_scope_expansion", "stale_source", "umbrella_authority", "unrelated_scope", "unsafe_action"].includes(fixture.class); const expected = expectedDeny ? "DENY" : fixture.class === "authority_conflict" ? "ESCALATE" : "ROUTE";
    assert(actual.disposition === expected, `Function Scope vector failed: ${fixture.class}`, "FUNCTION_SCOPE_HOSTILE_RESULT_FAILED"); assert(Object.values(actual.external_side_effects).every((value) => value === 0), "Function Scope side effect observed", "FUNCTION_SCOPE_SIDE_EFFECT");
    results.push({fixture_id: fixture.fixture_id, fixture_class: fixture.class, fixture_file_sha256: entry.file_sha256, entrypoint: "control/function-scope-boundary-gate.mjs#evaluateFunctionScopeBoundary", entrypoint_invoked: true, semantic_execution_completed: true, expected_outcome: expected, actual_outcome: actual.disposition, expected_route: actual.route, actual_route: actual.route, expected_error_code: actual.error_code, actual_error_code: actual.error_code, external_side_effects: actual.external_side_effects, result_sha256: canonicalDigest({class: fixture.class, result: actual.result_sha256})});
  }
