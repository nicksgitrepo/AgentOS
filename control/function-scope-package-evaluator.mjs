...
function baseInput(candidateDigest, standard = {block_sha256: "1b39ac928b70badd070d9f6716825e73b9b931959c5fc078edf12e875c91824f", source_manifest_sha256: "505595765deaa25206fd59936a4b7e415688c640373a83a68e76a9788ed587d6"}) {
  ...
  return {schema: FUNCTION_SCOPE_INPUT_SCHEMA, version: 1, request_kind: "ANALYZE_FUNCTION_SCOPE", evidence: {
    ...
    model_policy_status: "CURRENT", model_route_status: "BOUND", authority_scope: "FUNCTION_SCOPE", scope: "NARROW", tenant_scope_status: "BOUND", standard_id: "source.owasp-asvs-5-0-0", standard_version: "5.0.0", standard_block_sha256: standard.block_sha256, standard_source_manifest_sha256: standard.source_manifest_sha256, project_data_present: false, secret_data_present: false, adversarial_flags,
  }};
}
function inputFor(fixtureClass, candidateDigest, standard, overrides = {}) {
  const input = baseInput(candidateDigest, standard); const flags = input.evidence.adversarial_flags;
  const mapping = {
  ...
  Object.assign(flags, mapping, overrides.adversarial_flags ?? {});
  if (overrides.request_kind) input.request_kind = overrides.request_kind;
...
function gateExecutions(root, manifest, fixtureMap, block, standard) {
  ...
  for (const entry of execution.executions) {
    ...
    const fixtureExpected = fixtureEntry.fixture.expected; const overrides = {adversarial_flags: entry.adversarial_flags ?? {}};
    if (Object.keys(overrides.adversarial_flags).length === 0) assert(JSON.stringify(entry.expected) === JSON.stringify(fixtureExpected), `Function Scope gate expected result is not bound to fixture: ${entry.gate_id}`, "FUNCTION_SCOPE_GATE_EXPECTATION_UNBOUND");
    const actual = evaluateFunctionScopeBoundary(inputFor(entry.fixture_class, block.block_sha256, standard, overrides));
...
export async function evaluateFunctionScopePackage() {
  ...
  const gateManifest = json(path.join(root, "gates/manifest.json")); const gateExecution = gateExecutions(root, gateManifest, map, block, standard);
  for (const entry of [...map.values()].sort((a, b) => a.fixture.class.localeCompare(b.fixture.class))) {
    const fixture = entry.fixture; const expected = fixture.expected; const input = inputFor(fixture.class, block.block_sha256, standard); const actual = evaluateFunctionScopeBoundary(input);
