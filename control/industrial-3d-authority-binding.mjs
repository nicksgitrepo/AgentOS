...
function canonicalContext({blockSha, sourceManifestSha, standardSha, standardSourceSha, routerFileSha, routerBlockSha, routerFixtureSha, routerInputSha, routerResultSha, routerModelSnapshotSha, routerModelRouteSha, routerModelPolicyClaim, routerModelBindingStatus, modelSnapshotSha, modelRouteSha, fixtureContractSha, registry}) {
  const body = {
    ...
    registry_raw_file_sha256: registry.registry_raw_file_sha256,
    ...
    invalidation_triggers: [
      ...
      "registry raw bytes, projection, entry state, or semantic digest changes", "memory readback or context receipt changes", "custody or lifecycle state changes",
    ].sort(compareUtf8),
    context_sha256: null,
  };
  return Object.freeze({...body, context_sha256: canonicalDigest(body)});
}
function canonicalMemoryReadback(contextSha, registryRawFileSha256) {
  return Object.freeze({schema: "agentos.industrial_3d_memory_readback.v1", version: 1, status: "CURRENT", context_sha256: contextSha, registry_raw_file_sha256: registryRawFileSha256, policy: "READ_ONLY_CONTEXT_ONLY"});
}
...
function registryContract({blockSha, modelRoute}) {
  ...
  const registryRawFileSha256 = Object.freeze({
    agent_roster: agentArtifact.file_sha256,
    specialist_roster: specialistArtifact.file_sha256,
    atomic_inventory: atomicArtifact.file_sha256,
    routing_index: routingArtifact.file_sha256,
  });
  const registryContractSha256 = canonicalDigest({
    schema: "agentos.industrial_3d_registry_contract.v1", version: 1,
    registry_raw_file_sha256: registryRawFileSha256,
...
export function resolveIndustrial3dCanonicalAuthority() {
  ...
  const memoryReadback = canonicalMemoryReadback(context.context_sha256, registry.registry_raw_file_sha256);
