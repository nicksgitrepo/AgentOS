...
function validateInput(input) {
  ...
  for (const key of ["candidate_commit_sha1", "candidate_tree_sha1", "rollback_commit_sha1", "rollback_tree_sha1"]) { bounded(r[key], `review.${key}`, 40); assert(GIT_ID.test(r[key]) && !/^([0-9a-f])\1{39}$/u.test(r[key]), `review.${key} is not a Git identity`, "INDEPENDENT_AUDITOR_CANDIDATE_ID_INVALID"); }
