...
function makeNativeWorkerPrompt(lane, work) {
  return `${work.prompt} You have standing authority for serial repair work inside this admitted lane, worktree, branch, test boundary, and product intent. Ordinary edits, evidence corrections, test setup inside the boundary, bounded audit remediation, and successor candidates never require Controller, Orchestrator, Spawner, Sentinel, or Owner approval. Return only the typed native worker progress record for ${lane.lane_id}; the record must bind the exact campaign, source, native session, meaningful result, artifact digest, evidence digest, and observation time. Submit every heavyweight build, compile, test, verification, database, runtime, or artifact operation as one typed candidate-level plan to the shared AgentOS Hybrid Scheduler; never run competing heavyweight operations directly. Do not self-accept and do not perform protected delivery actions. Escalate only an evidence-complete condition that existing authority cannot resolve and that requires a concrete external decision.`;
}

function makeNativeAuditorPrompt(worker, handoff) {
  return `Independently audit the typed handoff digest ${handoff.handoff_sha256} for lane ${worker.lane_id}. A bounded FAIL returns directly to the same Repair worker and is never an Owner, Controller, Orchestrator, Spawner, or Sentinel decision. While Repair is legitimately waiting, maintain one next evidence-complete READY seam without releasing it early. Submit every heavyweight verification or evidence operation as one typed candidate-level plan to the shared AgentOS Hybrid Scheduler; never run competing heavyweight operations directly. Return only the typed native Auditor result bound to this handoff, the exact campaign source, an acceptance decision, and evidence digest. Do not modify the worker result, do not self-accept, and do not perform protected delivery actions. Escalate only an evidence-complete condition that existing authority cannot resolve and that requires a concrete external decision.`;
}
