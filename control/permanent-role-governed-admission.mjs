...
export function inspectCanonicalPermanentRoleCandidate({roleId} = {}) {
  ...
  const passed = ["EXECUTED_REVIEW_REQUIRED", "PASS"].includes(evaluation.disposition) && evaluation.results?.passed === 17 && evaluation.results?.failed === 0 && evaluation.results?.pending === 0 && evaluation.cases.every((entry) => entry.observed === "PASS");
