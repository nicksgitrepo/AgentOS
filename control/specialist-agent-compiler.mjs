...
function buildEvaluationReceipt({recipe, task, selectedBlocks, packageHash = null}) {
  ...
  return {schema: "agentos.evaluation_receipt.v1", version: 1, package_hash: packageHash, receipt_id: `agentos-evaluation.${recipe.recipe_id.replace(/[^a-z0-9-]+/giu, "-")}.v${recipe.version.split(".")[0]}`, model_requirement: "GLOBAL_MODEL_POLICY_SNAPSHOT/TASK_CLASS_ROUTE", independent_reviewer_required: true, disposition: "STATIC_PACKAGE_VALID;_INDEPENDENT_UTILITY_HARM_PENDING", tests, self_acceptance: "FORBIDDEN", mutation_rule: "CHANGED_APPLICABILITY_OR_LOCK_REQUIRES_NEW_PACKAGE_IDENTITY"};
}
