...
function inventory(root) {
  const files = ["block.json", "sources.lock", "gates/manifest.json", "gates/execution.json", "evaluation.json", "handoff.json", "audits/read-only-adversary-audit.v1.json"];
...
function validateCommittedOperationalArtifacts(root, authority, fixtureResults) {
  const evaluation = json(path.join(root, "evaluation.json")); const handoff = json(path.join(root, "handoff.json")); const audit = json(path.join(root, "audits/read-only-adversary-audit.v1.json"));
  assert(evaluation.schema === "agentos.specialist_evaluation.v1" && evaluation.version === 1 && evaluation.block_id === OWASP_API_OBJECT_AUTHORIZATION_BLOCK_ID && evaluation.candidate_digest === authority.block_sha256, "API1 static evaluation is not bound to the package", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_INVALID");
  assert(evaluation.evaluation_sha256 === canonicalDigest({...evaluation, evaluation_sha256: null}), "API1 static evaluation digest is invalid", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_DIGEST_INVALID");
  assert(Array.isArray(evaluation.cases) && evaluation.cases.length === fixtureResults.length, "API1 static evaluation case inventory is incomplete", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_INVALID");
  const observed = new Map(fixtureResults.map((entry) => [entry.fixture_id, entry]));
  for (const entry of evaluation.cases) { const result = observed.get(entry.case_id); assert(result && entry.class === result.fixture_class && entry.expected_disposition === result.expected_outcome && entry.actual_disposition === result.actual_outcome && entry.route === result.actual_route && entry.error_code === result.actual_error_code && entry.result_sha256 === result.result_sha256, `API1 static evaluation case is not executable-bound: ${entry.case_id}`, "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_UNBOUND"); }
  assert(evaluation.results?.passed === fixtureResults.length && evaluation.results.failed === 0 && evaluation.results.pending === 0 && evaluation.scope_fail_closed?.tenant_scope?.status === "DENY" && evaluation.scope_fail_closed?.object_scope?.status === "DENY" && evaluation.backend_evidence_guard?.fabricated_backend_evidence_allowed === false, "API1 static evaluation does not record fail-closed custody", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_INVALID");
  const modelBinding = evaluation.model_policy ?? evaluation.model_requirement;
  assert(modelBinding?.snapshot_sha256 === authority.model.snapshot_sha256 && modelBinding?.route_sha256 === authority.model_route_sha256, "API1 static evaluation model route is not bound", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_EVALUATION_UNBOUND");
  const freezeRoot = freezeRootDigest(inventory(root).map((relative_path) => ({relative_path: `${PACKAGE}/${relative_path}`, sha256: sha(fs.readFileSync(path.join(root, relative_path)))})));
  assert(evaluation.execution_receipts?.candidate_freeze_package_root_sha256 === freezeRoot && handoff.evidence_receipts?.candidate_freeze_package_root_sha256 === freezeRoot && audit.audited_candidate?.package_root_sha256 === freezeRoot, "API1 freeze root is not bound to the immutable package inventory", "OWASP_API_OBJECT_AUTHORIZATION_FREEZE_ROOT_INVALID");
  assert(handoff.schema === "agentos.specialist_handoff.v1" && handoff.version === 1 && handoff.block_id === OWASP_API_OBJECT_AUTHORIZATION_BLOCK_ID && handoff.candidate_digest === authority.block_sha256, "API1 handoff is not bound to the package", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_HANDOFF_INVALID");
  assert(handoff.scope?.tenant_scope_status === "BOUND" && handoff.scope?.object_scope_status === "BOUND" && handoff.scope?.scope_relation === "TENANT_OBJECT_BOUND", "API1 handoff scope is not fail-closed bound", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_HANDOFF_INVALID");
  assert(handoff.auditor_custody?.role === "INDEPENDENT_READ_ONLY_AUDITOR" && handoff.auditor_custody.write_allowed === false && handoff.auditor_custody.backend_access === false && handoff.auditor_custody.project_mutation_allowed === false, "API1 handoff auditor custody is not read-only", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_HANDOFF_INVALID");
  assert(handoff.backend_evidence?.status === "NOT_PROVIDED" && handoff.backend_evidence.claimed === false && handoff.backend_evidence.digest === null, "API1 handoff fabricates backend evidence", "OWASP_API_OBJECT_AUTHORIZATION_STATIC_HANDOFF_INVALID");
  assert(audit.schema === "agentos.specialist_read_only_adversary_audit.v1" && audit.version === 1 && audit.auditor?.role === "INDEPENDENT_READ_ONLY_AUDITOR" && audit.auditor.write_allowed === false && audit.auditor.backend_access === false && audit.auditor.project_mutation_allowed === false && audit.audit_sha256 === canonicalDigest({...audit, audit_sha256: null}), "API1 adversary audit is not a read-only self-bound receipt", "OWASP_API_OBJECT_AUTHORIZATION_AUDIT_INVALID");
  assert(audit.audited_candidate?.package_block_sha256 === authority.block_sha256 && /^[0-9a-f]{40}$/u.test(audit.audited_candidate.commit) && /^[0-9a-f]{40}$/u.test(audit.audited_candidate.tree) && handoff.source_commit === audit.audited_candidate.commit && handoff.source_tree === audit.audited_candidate.tree, "API1 adversary audit does not bind the handoff to an exact commit/tree", "OWASP_API_OBJECT_AUTHORIZATION_AUDIT_BINDING_INVALID");
  return Object.freeze({evaluation, handoff, audit});
}
