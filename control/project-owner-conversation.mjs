...
export const HUMAN_FACING_ROLE = "AGENTOS.PRODUCT_OWNER";
export const DEFAULT_EXPLANATION_LEVEL = "SIMPLE";
export const EXPLANATION_LEVELS = Object.freeze(["SIMPLE", "ELABORATE", "ADVANCED"]);

const HASH_OR_CODE = /(?:\b[0-9a-f]{12,64}\b|\b[A-Z][A-Z0-9_]*(?:\.[A-Z0-9_]+){1,}\b|\b[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+\b|\b(?:sha256|commit|tree|digest|schema|stack trace)\b)/u;
...
function simpleVisibleText(value, label) {
  text(value, label);
  assert(!HASH_OR_CODE.test(value) && !TECHNICAL_FENCE.test(value), `${label} exposes codes, hashes, paths, or debugging details`);
}
...
export function validateProjectOwnerConversation(value) {
  exact(value, ["schema", "version", "authority_status", "speaker_role", "explanation_level", "message", "choices", "technical_details_hidden", "private_evidence_refs", "conversation_sha256"], "Project Owner conversation");
  assert(value.schema === PROJECT_OWNER_CONVERSATION_SCHEMA && value.version === 1, "Project Owner conversation identity differs");
  assert(value.authority_status === "NON_AUTHORITATIVE_TEMPLATE", "Structural Product Owner conversation cannot grant operational authority");
  ...
  const observedKeys = value.choices.map((choice) => choice?.key);
  const numericKeys = ["1", "2", "3", "4", "n"].slice(0, value.choices.length);
  const letterKeys = ["a", "b", "c", "d", "n"].slice(0, value.choices.length);
  assert(new Set(observedKeys).size === observedKeys.length && (JSON.stringify(observedKeys) === JSON.stringify(numericKeys) || JSON.stringify(observedKeys) === JSON.stringify(letterKeys)), "Project Owner choice keys must be one unique ordered number or letter set");
  for (const [index, choice] of value.choices.entries()) {
    ...
    simpleVisibleText(choice.label, `Project Owner choice ${index} label`);
    simpleVisibleText(choice.meaning, `Project Owner choice ${index} meaning`);
  }
