...
export function assertIntentRegulatorCommittedHandoff({authority = resolveIntentRegulatorCanonicalAuthority(), evaluation, handoff, evaluationFileSha256, handoffFileSha256} = {}) {
  ...
  assert(evaluation.schema === "agentos.specialist_evaluation.v1" && evaluation.version === 1 && evaluation.receipt_id === "specialist-eval.intent-regulator.v1" && evaluation.block_id === INTENT_REGULATOR_BLOCK_ID && evaluation.candidate_digest === authority.block_sha256 && evaluation.results?.passed === 0 && evaluation.results?.failed === 0 && evaluation.results?.pending === INTENT_REGULATOR_FIXTURE_CLASSES.length && evaluation.disposition === "UTILITY_HARM_PENDING", "Intent-regulator evaluation dossier is not a complete candidate receipt", "INTENT_REGULATOR_EVALUATION_INVALID");
  assert(evaluation.cases.length === INTENT_REGULATOR_FIXTURE_CLASSES.length && evaluation.cases.every((entry) => INTENT_REGULATOR_FIXTURE_CLASSES.includes(entry.class) && entry.observed === "PENDING"), "Intent-regulator evaluation case coverage is incomplete", "INTENT_REGULATOR_EVALUATION_INVALID");
