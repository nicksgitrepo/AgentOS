...
function score(block, tier, role, repositoryRoot = process.cwd()) {
  ...
  const finishCost = role && !exists(repositoryRoot, role.package_path + "/block.json") ? 95 : block.role_kind === "STANDARD_BLOCK" ? 35 : 45;
...
function operationalEntrypoints(root, relativePath, evaluation) {
  if (!evaluation?.boundary_entrypoint || !evaluation?.evaluator_entrypoint) return undefined;
  const gateExecution = `${relativePath}/gates/execution.json`;
  return {
    boundary: evaluation.boundary_entrypoint,
    evaluator: evaluation.evaluator_entrypoint,
    ...(exists(root, gateExecution) ? {gate_execution: gateExecution} : {}),
    real_fixture_inputs: evaluation.fixture_policy === "EXACT_DECLARED",
    mutation_sensitivity_required: typeof evaluation.harness === "string" && evaluation.harness.includes("executable"),
    read_only: true,
  };
}
function contextInvalidation(block) {
  if (!Array.isArray(block.context_binding?.invalidate_on) || block.context_binding.invalidate_on.length === 0 || !block.memory_binding?.policy || !block.invalidation) return undefined;
  return {
    status: block.invalidation.status,
    memory_rule: block.memory_binding.policy,
    rebuild_on_change: true,
    invalidate_on: [...(block.context_binding.invalidate_on ?? [])],
    rebuild_rule: block.invalidation.rebuild_rule,
  };
}
function compileEntry(root, relativePath, acceptanceById = new Map()) {
  ...
  const triggers = [...(block.routing?.signals ?? []), ...(block.intake?.required_context ?? []), ...(block.applicability_inputs ?? [])].map(text).filter(Boolean).slice(0, 20); const knowledge = [...new Set([...(block.required_knowledge ?? []), ...(block.intake?.required_context ?? []), ...sourceIds].map(text).filter(Boolean))];
...
export function compileReusableAgentRoster({repositoryRoot = process.cwd(), writeGenerated = false} = {}) {
  ...
  for (const [id, role] of Object.entries(PERMANENT_ROLES)) if (!entries.some((entry) => entry.stable_agent_id === id)) entries.push(missingPermanentEntry(id, role, repositoryRoot));
