...
function missingPermanentEntry(id, role, repositoryRoot) {
...
export function compileReusableAgentRoster({repositoryRoot = process.cwd(), writeGenerated = false} = {}) {
  resolveSpawnerRuntimeCustody({projectRoot: repositoryRoot, repositoryRoot, taskWorktreePath: repositoryRoot});
  ...
  const nextEligible = modelPolicyFresh ? orderedAgents.find((entry) => {
  ...
  const build_queue = orderedAgents.map((entry, index) => ({rank: index + 1, stable_agent_id: entry.stable_agent_id, tier: entry.tier, eligible: entry.stable_agent_id === nextEligible, priority_score: entry.reuse_likelihood.total, reason: !modelPolicyFresh ? "Blocked until the current model-policy snapshot and evidence store are fresh; no package may advance." : entry.build_state === "ACCEPTED_ADMITTED" || entry.build_state === "ACCEPTED_QUALIFIED" ? "Already independently accepted; reuse its exact receipt until invalidated." : entry.stable_agent_id === nextEligible ? "Highest eligible unaccepted package after dependency-safe predecessors." : entry.build_state === "PLANNED_MISSING_PACKAGE" ? "Blocked until the canonical package is built and qualified." : entry.tier === "PERMANENT_AGENTOS_ROLES" ? "Waits for dependency-safe permanent-role predecessors." : "Queued after permanent roles and platform prerequisites."}));
