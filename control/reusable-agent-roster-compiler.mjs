...
function operationalReadback(root, evaluation, block) {
  const declared = evaluation?.operational_readback;
  if (!declared || typeof declared.path !== "string" || path.isAbsolute(declared.path) || declared.path.split("/").includes("..") || !exists(root, declared.path)) return null;
  try {
    const readback = readJson(root, declared.path);
    if (readback.status !== "PASS" || readback.block_id !== block.block_id || readback.candidate_digest !== block.block_sha256 || !/^[0-9a-f]{64}$/u.test(readback.readback_sha256) || readback.readback_sha256 !== canonicalDigest({...readback, readback_sha256: null}) || fileSha(root, declared.path) !== declared.file_sha256) return null;
    return readback;
  } catch {
    return null;
  }
...
function compileEntry(root, relativePath, acceptanceById = new Map()) {
  ...
  const evaluationPath = exists(root, `${relativePath}/evaluation.json`) ? `${relativePath}/evaluation.json` : null; const evaluation = evaluationPath ? readJson(root, evaluationPath) : null; const operational = operationalReadback(root, evaluation, block); const evaluationState = evaluation?.operational_readback ? (operational ? evaluation.disposition : "STATIC_PASS_REVIEW_REQUIRED") : evaluation?.disposition; const handoffPath = exists(root, `${relativePath}/handoff.json`) ? `${relativePath}/handoff.json` : null;
  ...
  const qaState = role?.id === "AGENTOS.SPAWNER" || acceptedReadback ? "COMPLETE_QA_PASS" : missing ? "MISSING_PACKAGE" : evaluationState ?? "STATIC_PASS_REVIEW_REQUIRED"; const independentState = role?.id === "AGENTOS.SPAWNER" ? "ACCEPTED_CLEARANCE_BOUND" : acceptedReadback ? "INDEPENDENT_PASS_READBACK" : archived ? "RETIRED_READ_ONLY" : evaluationState ?? "PENDING_EXTERNAL_REVIEW";
  const links = ["supersession.json", "compatibility.json", "evaluation.json"].filter((file) => exists(root, `${relativePath}/${file}`)).map((file) => `${relativePath}/${file}`); if (evaluation?.operational_readback?.path && !links.includes(evaluation.operational_readback.path)) links.push(evaluation.operational_readback.path);
...
export function compileReusableAgentRoster({repositoryRoot = process.cwd(), writeGenerated = false} = {}) {
  ...
  const nextEligible = orderedAgents.find((entry) => {
    ...
    if (entry.qa_state !== "STATIC_PASS_REVIEW_REQUIRED" || entry.independent_evaluation_state !== "STATIC_PASS_REVIEW_REQUIRED") return false;
  ...
  const build_queue = orderedAgents.map((entry, index) => ({rank: index + 1, stable_agent_id: entry.stable_agent_id, tier: entry.tier, eligible: entry.stable_agent_id === nextEligible, priority_score: entry.reuse_likelihood.total, reason: entry.build_state === "ACCEPTED_ADMITTED" || entry.build_state === "ACCEPTED_QUALIFIED" ? "Already independently accepted; reuse its exact receipt until invalidated." : entry.qa_state === "EXECUTED_REVIEW_REQUIRED" || entry.independent_evaluation_state === "EXECUTED_REVIEW_REQUIRED" ? "Frozen after local execution; waits for fresh external authority and read-only Luna audit." : entry.stable_agent_id === nextEligible ? "Highest eligible unaccepted package after dependency-safe predecessors." : entry.build_state === "PLANNED_MISSING_PACKAGE" ? "Blocked until the canonical package is built and qualified." : entry.tier === "PERMANENT_AGENTOS_ROLES" ? "Waits for dependency-safe permanent-role predecessors." : "Queued after permanent roles and platform prerequisites."}));
