...
function usesStaticSemanticRegistryProvenance(block) {
  return block.registry_provenance?.mode === "STATIC_SEMANTIC_ONLY";
}
function fixtureInventory(root, relativePath, block) {
  const staticProvenance = usesStaticSemanticRegistryProvenance(block);
  ...
  if (exists(root, manifestPath)) {
    ...
    return {status: "BOUND", fixtures: (manifest.entries ?? []).flatMap((entry) => {
      ...
      return exists(root, fixturePath) ? [{fixture_id: entry.fixture_id, path: fixturePath, ...(staticProvenance ? {} : {file_sha256: fileSha(root, fixturePath)}), expected_outcome: entry.expected_outcome ?? entry.expected ?? "DENY"}] : [];
    })};
  }
  ...
  const fixtures = fs.readdirSync(fixtureDirectory).filter((name) => name.endsWith(".json")).sort().map((name) => {
    ...
    const expectedOutcome = fixture.vector?.expected_readback?.disposition
      ...
      ?? "DENY";
    return {fixture_id: fixture.fixture_id ?? `${block.block_id}.${normalize(name.replace(/\.json$/u, ""))}`, path: fixturePath, ...(staticProvenance ? {} : {file_sha256: fileSha(root, fixturePath)}), expected_outcome: expectedOutcome};
  });
