...
const TRUST_ANCHOR_URL = new URL("../specialist-blocks/control-plane/agent-spawner/independent-clearance-trust-anchor.v1.json", import.meta.url);
...
export function loadIndependentClearanceTrustAnchor() {
  const anchor = readJson(fs.realpathSync.native(TRUST_ANCHOR_URL), "Independent-clearance trust anchor");
...
export function validateIndependentEvaluatorRegistry(registry, {anchor, nowUtc} = {}) {
...
function verifyBundle({authorityRoot, repositoryRoot, receiptSha256, consume, issueToken}) {
  ...
  const registry = validateIndependentEvaluatorRegistry(readJson(within(authorityRoot, "evaluator-registry.v2.json", "Evaluator registry"), "Evaluator registry"), {anchor, nowUtc});
...
export function verifyIndependentSpawnerClearance({authorityRoot, receiptSha256} = {}) { assert(typeof authorityRoot === "string" && path.isAbsolute(authorityRoot), "Independent-clearance authority root must be an absolute governed path"); return verifyBundle({authorityRoot: fs.realpathSync.native(authorityRoot), repositoryRoot: REPOSITORY_ROOT, receiptSha256, consume: true, issueToken: true}).clearance; }
