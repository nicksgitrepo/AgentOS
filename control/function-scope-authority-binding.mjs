...
export function resolveFunctionScopeCanonicalAuthority() {
  ...
  const gates = GATE_IDS.map((gateId, index) => {
    ...
    if (entry.deterministic_gates?.gates?.[index]) assert(entry.deterministic_gates.gates[index].file_sha256 === artifact.file_sha256, `Function Scope gate ${gateId} differs from the canonical roster`, "FUNCTION_SCOPE_GATE_PROVENANCE_INVALID");
  ...
  const fixtures = fixtureNames.map((name) => {
    ...
    const rosterFixture = entry.hostile_fixtures?.fixtures?.find((candidate) => candidate.path === `${FUNCTION_SCOPE_PACKAGE_PATH}/fixtures/${name}`);
    if (rosterFixture) assert(rosterFixture.file_sha256 === artifact.file_sha256, `Function Scope fixture ${name} differs from the canonical roster`, "FUNCTION_SCOPE_FIXTURE_PROVENANCE_INVALID");
  ...
  const contextSha256 = canonicalDigest({block_sha256: block.block_sha256, source_manifest_sha256: source.manifest_sha256, standard_block_sha256: standard.block_sha256, standard_source_manifest_sha256: standardSources.manifest_sha256, authority_scope: "FUNCTION_SCOPE", scope: "NARROW", tenant_scope_status: "BOUND", custody_ref: FUNCTION_SCOPE_CUSTODY_REF, router_result_sha256: routerResult.result_sha256, model_route_sha256: modelRouteSha256});
  return Object.freeze({
    ...
    router_result_sha256: routerResult.result_sha256, context_sha256: contextSha256, custody_ref: FUNCTION_SCOPE_CUSTODY_REF,
  });
}
