...
export const AGENTOS_CONTROLLER_DISPLAY_NAME = "AgentOS Controller";
...
export function validateControllerRoleDisplay({controllerRole, controllerDisplayName, displayTitle = null}, {taskId = null, label = "Controller role display"} = {}) {
  ...
  assert(controllerDisplayName === AGENTOS_CONTROLLER_DISPLAY_NAME, `${label} must use AgentOS Controller`);
  if (displayTitle !== null) {
    ...
    else assert(displayTitle.startsWith(`${AGENTOS_CONTROLLER_DISPLAY_NAME} — `), `${label} title does not identify AgentOS Controller`);
  }
