 ...
 *
 * This module is deliberately self contained: issue state is typed, every
 * transition is content addressed, and the Registrar is the only role allowed
 * to materialise the canonical issues.md projection.  The implementation does
 * not inspect a task database or invent evidence; callers provide immutable
 * evidence references and the registrar preserves them verbatim (redacted to
 * opaque references when marked sensitive).
 */
...
export const ISSUE_REGISTRAR_SCHEMA = "agentos.issue_registrar.v1";
export const ISSUE_REGISTRAR_VERSION = 1;
export const ISSUE_REGISTRAR_ROLE_ID = "AGENTOS.ISSUE_REGISTRAR";
...
export const ISSUE_SEVERITIES = Object.freeze(["BLOCKER", "HIGH", "MEDIUM", "LOW", "INFO"]);
export const ISSUE_TERMINAL_OWNER_STATUSES = Object.freeze(["DEFERRED", "ACCEPTED_RISK", "WONT_FIX"]);
export const ISSUE_REGISTRY_SCHEMA = "agentos.issue_registry.v1";
export const ISSUE_SUBMISSION_SCHEMA = "agentos.issue_submission.v1";
export const ISSUE_TRANSITION_SCHEMA = "agentos.issue_transition.v1";

const SHA256 = /^[0-9a-f]{64}$/u;
const HEX40 = /^[0-9a-f]{40}$/u;
const PRODUCT_PREFIX = /^[A-Z][A-Z0-9]{1,31}$/u;
const ISSUE_ID = /^([A-Z][A-Z0-9]{1,31})-ISSUE-(\d{4})-(\d{4,})$/u;
...
function requireIdentifier(value, label, code = "ISSUE_REGISTRAR_INVALID_IDENTIFIER") {
  assert(typeof value === "string" && /^[A-Z][A-Z0-9._:-]{0,191}$/u.test(value), code, `${label} must be a stable identifier`);
  assert(!CONTROL.test(value), code, `${label} contains control characters`);
}

function requireText(value, label, minimum = 1, code = "ISSUE_REGISTRAR_INVALID_TEXT") {
  assert(typeof value === "string" && value.trim().length >= minimum, code, `${label} must be non-empty text`);
  assert(!CONTROL.test(value), code, `${label} contains control characters`);
}
...
function sortedUnique(values, label) {
  assert(Array.isArray(values), "ISSUE_REGISTRAR_INVALID_ARRAY", `${label} must be an array`);
  ...
  assert(JSON.stringify(values) === JSON.stringify(ordered), "ISSUE_REGISTRAR_NONDETERMINISTIC_ORDER", `${label} must be sorted`);
  return values;
}
...
function timestamp(options = {}) {
  const value = options.nowUtc ?? options.now_utc ?? new Date().toISOString();
  requireUtc(value, "Registrar timestamp");
  return value;
}

function normalizePrefix(value, fallback = "SOCIUNA") {
  const prefix = String(value ?? fallback).trim().toUpperCase();
  assert(PRODUCT_PREFIX.test(prefix), "ISSUE_REGISTRAR_INVALID_PRODUCT_PREFIX", "product prefix is invalid");
  return prefix;
}

function normalizeYear(value, fallback = new Date().getUTCFullYear()) {
  const year = Number(value ?? fallback);
  assert(Number.isInteger(year) && year >= 2000 && year <= 9999, "ISSUE_REGISTRAR_INVALID_YEAR", "issue year is invalid");
  return year;
}

function normalizeReporter(input = {}) {
  const source = isRecord(input.reporter) ? input.reporter : input;
  return {
...
function normalizeEvidence(values) {
  ...
  assert(Array.isArray(values), "ISSUE_REGISTRAR_INVALID_EVIDENCE", "evidence must be an array");
  const result = values.map((entry, index) => {
    const source = isRecord(entry) ? entry : {kind: "UNSTRUCTURED", payload: entry};
    const evidenceId = asOptionalString(source.evidence_id ?? source.evidenceId) ?? `EVIDENCE.${String(index + 1).padStart(4, "0")}`;
    requireIdentifier(evidenceId, "evidence ID");
    const kind = String(source.kind ?? "UNSPECIFIED").trim().toUpperCase().replace(/[^A-Z0-9._:-]/gu, "_");
    requireIdentifier(kind, "evidence kind");
    ...
    const suppliedReference = asOptionalString(source.reference ?? source.ref);
    const reference = sensitive || !suppliedReference || !REFERENCE.test(suppliedReference)
  ...
  const ids = result.map((entry) => entry.evidence_id);
  assert(new Set(ids).size === ids.length, "ISSUE_REGISTRAR_DUPLICATE_EVIDENCE", "evidence IDs must be unique");
  return result;
}

function normalizeRelated(values) {
  if (values === undefined || values === null) return [];
  assert(Array.isArray(values), "ISSUE_REGISTRAR_INVALID_RELATION", "related issue IDs must be an array");
  const result = values.map((value) => String(value)).sort(compareUtf8);
  result.forEach((value) => assert(ISSUE_ID.test(value), "ISSUE_REGISTRAR_INVALID_RELATION", "related issue ID is invalid"));
...
function normalizeCandidate(value) {
  ...
  if (candidate.parent !== null) assert(HEX40.test(candidate.parent) || SHA256.test(candidate.parent), "ISSUE_REGISTRAR_INVALID_CANDIDATE", "candidate parent must be a digest");
  return candidate;
}

function normalizeHistory(history, nowUtc, event, reason, actor = ISSUE_REGISTRAR_ROLE_ID) {
  const existing = Array.isArray(history) ? history.map(clone) : [];
  const sequence = existing.length + 1;
  existing.push({sequence, event, at_utc: nowUtc, actor, reason: reason ?? ""});
  return existing;
}
...
function nextNumber(registry, prefix, year) {
  ...
  let number = Number(registry.next_numbers?.[key] ?? 1);
  while (used.has(number)) number += 1;
  return {key, number};
}

function dedupeKey(input, prefix) {
  if (typeof input.dedupe_key === "string" && input.dedupe_key.trim().length > 0) return input.dedupe_key.trim();
  const title = String(input.title ?? "").trim().toLocaleLowerCase();
  const category = String(input.category ?? "").trim().toLocaleLowerCase();
  const root = String(input.root_issue_id ?? input.rootIssueId ?? "").trim();
  return `${prefix}|${category}|${title}|${root}`;
}
...
function validateReporter(reporter) {
  assert(isRecord(reporter), "ISSUE_REGISTRAR_INVALID_REPORTER", "reporter must be an object");
  const expected = ["identity", "item_id", "task_id", "thread_id", "turn_id"];
...
export function validateIssueRecord(issue) {
  ...
  assert(Number.isInteger(issue.number) && issue.number > 0, "ISSUE_REGISTRAR_INVALID_NUMBER", "issue number is invalid");
  const parsed = ISSUE_ID.exec(issue.issue_id);
  ...
  for (const field of ["accepted_fields", "missing_fields", "invalid_fields", "resubmission_requirements", "related_issue_ids", "evidence"]) {
    assert(Array.isArray(issue[field]), "ISSUE_REGISTRAR_INVALID_RECORD", `issue ${field} must be an array`);
    const strings = field === "evidence" ? null : issue[field];
    if (strings !== null) {
      strings.forEach((value) => requireText(value, `issue ${field} item`));
      sortedUnique(strings, `issue ${field}`);
    }
  }
  for (const evidence of issue.evidence) {
    assert(isRecord(evidence), "ISSUE_REGISTRAR_INVALID_EVIDENCE", "evidence must be an object");
    requireIdentifier(evidence.evidence_id, "evidence ID");
    requireIdentifier(evidence.kind, "evidence kind");
    assert(typeof evidence.reference === "string" && REFERENCE.test(evidence.reference), "ISSUE_REGISTRAR_INVALID_EVIDENCE", "evidence reference is invalid");
    requireSha(evidence.sha256, "evidence digest");
  ...
  assert(Array.isArray(issue.history) && issue.history.length > 0, "ISSUE_REGISTRAR_INVALID_HISTORY", "issue history is required");
  issue.history.forEach((entry, index) => {
    assert(isRecord(entry) && entry.sequence === index + 1, "ISSUE_REGISTRAR_INVALID_HISTORY", "issue history sequence is not canonical");
    requireText(entry.event, "history event");
...
export function compileIssueRegistry({productPrefixes = ["SOCIUNA"], reservations = [], nextNumbers = {}, issues = [], duplicateSubmissions = []} = {}) {
  const prefixes = [...new Set(productPrefixes.map((value) => normalizePrefix(value)))].sort(compareUtf8);
  const orderedIssues = issues.map((item) => {
    const copy = clone(item);
    validateIssueRecord(copy);
    return copy;
  }).sort((left, right) => compareUtf8(left.issue_id, right.issue_id));
  const registry = {
    schema: ISSUE_REGISTRY_SCHEMA,
    version: ISSUE_REGISTRAR_VERSION,
  ...
  for (const reservation of registry.reservations) {
    requireIdentifier(reservation.reservation_id, "reservation ID");
    assert(PRODUCT_PREFIX.test(reservation.product_prefix), "ISSUE_REGISTRAR_INVALID_RESERVATION", "reservation prefix is invalid");
  ...
  requireSha("0".repeat(64), "registry digest");
  registry.registry_sha256 = digestWithout(registry, "registry_sha256");
  return validateIssueRegistry(registry);
}
...
export function validateIssueRegistry(registry) {
  ...
  assert(Array.isArray(registry.issues), "ISSUE_REGISTRAR_INVALID_REGISTRY", "registry issues are required");
  const ids = registry.issues.map((item) => item.issue_id);
  assert(new Set(ids).size === ids.length, "ISSUE_REGISTRAR_DUPLICATE_ID", "issue IDs must be unique");
  const ordered = [...ids].sort(compareUtf8);
  assert(JSON.stringify(ids) === JSON.stringify(ordered), "ISSUE_REGISTRAR_NONDETERMINISTIC_ORDER", "registry issues are not sorted");
  registry.issues.forEach(validateIssueRecord);
  assert(Array.isArray(registry.reservations), "ISSUE_REGISTRAR_INVALID_REGISTRY", "registry reservations are required");
  assert(Array.isArray(registry.duplicate_submissions), "ISSUE_REGISTRAR_INVALID_REGISTRY", "duplicate submissions are required");
  requireSha(registry.registry_sha256, "registry digest");
  assert(registry.registry_sha256 === digestWithout(registry, "registry_sha256"), "ISSUE_REGISTRAR_DIGEST_MISMATCH", "registry digest mismatch");
  return registry;
}

export function reserveIssueNumber(registry, {productPrefix = "SOCIUNA", year = normalizeYear(), reservationId = null, nowUtc = undefined} = {}) {
  const next = nextNumber(registry, normalizePrefix(productPrefix), normalizeYear(year));
  const reserved = clone(registry);
  reserved.reservations ??= [];
  reserved.next_numbers ??= {};
  const id = reservationId ?? `RESERVATION.${next.key}.${String(next.number).padStart(4, "0")}`;
  requireIdentifier(id, "reservation ID");
...
function buildSubmissionRecord(input, {prefix, year, number, nowUtc, reservationId, historical = false} = {}) {
  const title = asOptionalString(input.title ?? input.name);
  const summary = asOptionalString(input.summary ?? input.description ?? input.details);
  const category = asOptionalString(input.category ?? input.classification);
  ...
  const missing = [];
  const invalid = [];
  if (title) accepted.push("title"); else missing.push("title");
  if (summary) accepted.push("summary"); else missing.push("summary");
  if (category) accepted.push("category"); else missing.push("category");
  ...
  if (reporterPresent(reporter)) accepted.push("reporter"); else missing.push("reporter");
  if (evidence.length > 0) accepted.push("evidence"); else missing.push("evidence");
  const complete = missing.length === 0 && invalid.length === 0;
  const status = complete ? "READY" : "INTAKE_FAILED";
  const lifecycleStage = complete ? "READY" : "NOT_AUTHORIZED";
  const issue = {
  ...
  if (issue.regression_of !== null) {
    assert(ISSUE_ID.test(issue.regression_of), "ISSUE_REGISTRAR_INVALID_RELATION", "regression_of is invalid");
...
export function submitIssue(registry = compileIssueRegistry(), input = {}, options = {}) {
  ...
  assert(isRecord(input), "ISSUE_REGISTRAR_INVALID_SUBMISSION", "submission must be an object");
  const nowUtc = timestamp(options);
  const prefix = normalizePrefix(input.product_prefix ?? input.productPrefix ?? options.productPrefix);
  const year = normalizeYear(input.year ?? options.year);
  const key = dedupeKey(input, prefix);
  const existing = registry.issues.find((item) => item.dedupe_key === key && item.product_prefix === prefix);
  if (existing) {
    const duplicate = clone(existing);
    const reportIndex = duplicate.duplicate_reports.length + 1;
    duplicate.duplicate_reports.push({
    ...
    duplicate.history = normalizeHistory(duplicate.history, nowUtc, "DUPLICATE_LINKED", `Linked duplicate submission to ${duplicate.issue_id}.`);
    duplicate.updated_at_utc = nowUtc;
    duplicate.issue_sha256 = digestWithout(duplicate, "issue_sha256");
    const issues = registry.issues.filter((item) => item.issue_id !== duplicate.issue_id).concat(duplicate);
...
function actorRole(actor) {
  ...
  return actor.role ?? actor.role_id ?? actor.identity ?? null;
}

function assertOwner(actor) {
  const role = actorRole(actor);
  assert(["OWNER", "PROJECT_OWNER", "AGENTOS.PROJECT_OWNER"].includes(role), "ISSUE_REGISTRAR_OWNER_ONLY", "terminal issue decisions require Project Owner authority");
}

function patchIssue(issue, patch, nowUtc, event, actor = ISSUE_REGISTRAR_ROLE_ID) {
  const next = clone(issue);
  for (const field of ["title", "summary", "category"]) if (patch[field] !== undefined) {
    requireText(patch[field], `issue ${field}`);
    next[field] = patch[field].trim();
  }
  if (patch.severity !== undefined) {
    const severity = String(patch.severity).toUpperCase();
    assert(ISSUE_SEVERITIES.includes(severity), "ISSUE_REGISTRAR_INVALID_SEVERITY", "issue severity is invalid");
  ...
  if (patch.status !== undefined) {
    ...
    if (ISSUE_TERMINAL_OWNER_STATUSES.includes(status)) assertOwner(actor);
    next.status = status;
    next.lifecycle_stage = status === "INTAKE_FAILED" ? "NOT_AUTHORIZED" : status;
  ...
  if (patch.regression_of !== undefined) {
    assert(ISSUE_ID.test(String(patch.regression_of)), "ISSUE_REGISTRAR_INVALID_RELATION", "regression_of is invalid");
    next.regression_of = String(patch.regression_of);
    next.status = "REGRESSION";
    next.lifecycle_stage = "REGRESSION";
  }
  if (patch.supersedes !== undefined) {
    assert(ISSUE_ID.test(String(patch.supersedes)), "ISSUE_REGISTRAR_INVALID_RELATION", "supersedes is invalid");
    next.supersedes = String(patch.supersedes);
    next.status = "SUPERSEDED";
    next.lifecycle_stage = "SUPERSEDED";
  }
  next.updated_at_utc = nowUtc;
  next.history = normalizeHistory(next.history, nowUtc, event, patch.reason ?? `Issue transition ${event}.`, actorRole(actor) ?? ISSUE_REGISTRAR_ROLE_ID);
  next.issue_sha256 = digestWithout(next, "issue_sha256");
  return validateIssueRecord(next);
}

export function completeIssue(registry, issueIdValue, fields = {}, {nowUtc, actor = ISSUE_REGISTRAR_ROLE_ID} = {}) {
  validateIssueRegistry(registry);
  const found = registry.issues.find((item) => item.issue_id === issueIdValue);
  assert(found, "ISSUE_REGISTRAR_NOT_FOUND", "issue ID does not exist");
  ...
  next.issue_id = found.issue_id;
  next.root_issue_id = found.root_issue_id ?? found.issue_id;
  if (next.missing_fields.length === 0 && next.invalid_fields.length === 0) {
    next.status = "READY";
    next.lifecycle_stage = "READY";
    next.failure_code = null;
  } else {
    next.status = "INTAKE_FAILED";
    next.lifecycle_stage = "NOT_AUTHORIZED";
    next.failure_code = ISSUE_REGISTRAR_FAILURE_CODE;
  }
  next.updated_at_utc = timestamp({nowUtc});
  next.issue_sha256 = digestWithout(next, "issue_sha256");
...
export function submitRegression(registry, input, regressionOf, options = {}) {
  assert(ISSUE_ID.test(regressionOf), "ISSUE_REGISTRAR_INVALID_RELATION", "regression_of is invalid");
...
export function validateIssueAuditAdmission({issue, candidate} = {}) {
  validateIssueRecord(issue);
  assert(ISSUE_ID.test(issue.issue_id), "ISSUE_REGISTRAR_AUDIT_ID_REQUIRED", "audit requires a registered issue ID");
...
export function validateIssueRuntimeDelivery({issue, candidate, independentPass, delivery} = {}) {
  validateIssueRecord(issue);
  validateIssueAuditAdmission({issue, candidate});
...
export function importHistoricalIssues(sources, {registry = compileIssueRegistry(), productPrefix = "SOCIUNA", year = normalizeYear(), nowUtc} = {}) {
  assert(Array.isArray(sources), "ISSUE_REGISTRAR_HISTORICAL_INPUT_REQUIRED", "historical sources must be an array");
  ...
  for (const source of [...sources].sort((left, right) => compareUtf8(String(left.source_id ?? left.sourceId ?? ""), String(right.source_id ?? right.sourceId ?? "")))) {
    assert(isRecord(source), "ISSUE_REGISTRAR_HISTORICAL_INVALID", "historical source must be an object");
    const result = submitIssue(current, {
      ...source,
    ...
    }, {nowUtc, historical: true, productPrefix, year});
    current = result.registry;
    imported.push(result.issue.issue_id);
  }
...
export function reconcileIssueRegistry(registry) {
  ...
  for (const issue of registry.issues) {
    assert(!issueIds.has(issue.issue_id), "ISSUE_REGISTRAR_DUPLICATE_ID", "reconciliation found duplicate issue ID");
...
export function compileIssueRegistrarRole({blockSha256 = canonicalDigest({role: ISSUE_REGISTRAR_ROLE_ID}), evaluationSha256 = canonicalDigest({role: ISSUE_REGISTRAR_ROLE_ID, evaluation: true})} = {}) {
  requireSha(blockSha256, "Issue Registrar block digest");
  requireSha(evaluationSha256, "Issue Registrar evaluation digest");
  const role = {
    schema: ISSUE_REGISTRAR_SCHEMA,
    version: ISSUE_REGISTRAR_VERSION,
  ...
  return validateIssueRegistrarRole(role);
}
...
export function validateIssueRegistrarRole(role) {
  ...
  requireSha(role.block_sha256, "Issue Registrar block digest");
  requireSha(role.evaluation_sha256, "Issue Registrar evaluation digest");
  ...
  requireSha(role.role_sha256, "Issue Registrar role digest");
  assert(role.role_sha256 === digestWithout(role, "role_sha256"), "ISSUE_REGISTRAR_DIGEST_MISMATCH", "Issue Registrar role digest mismatch");
  return role;
}
...
export class IssueRegistrar {
  ...
  submit(input, options = {}) {
    const result = submitIssue(this.registry, input, options);
    this.registry = result.registry;
    return result;
  }

  complete(issueIdValue, fields, options = {}) {
    const result = completeIssue(this.registry, issueIdValue, fields, {actor: options.actor ?? this.writerIdentity, nowUtc: options.nowUtc});
    this.registry = result.registry;
    return result;
  }

  update(issueIdValue, patch, options = {}) {
    const result = updateIssue(this.registry, issueIdValue, patch, {actor: options.actor ?? this.writerIdentity, nowUtc: options.nowUtc});
    this.registry = result.registry;
    return result;
  }

  importHistorical(sources, options = {}) {
    const result = importHistoricalIssues(sources, {...options, registry: this.registry});
    this.registry = result.registry;
    return result;
  }

  reconcile() {
    this.registry = reconcileIssueRegistry(this.registry);
...
export const compileIssueSubmission = submitIssue;
export const validateIssueSubmission = validateIssueRecord;
export const compileIssueTransition = updateIssue;
export const validateIssueTransition = updateIssue;
