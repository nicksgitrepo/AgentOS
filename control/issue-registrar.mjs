#!/usr/bin/env node
import fs from "node:fs"; import path from "node:path"; import {canonicalDigest, compareUtf8} from "./content-addressing.mjs"; import {BLUEPRINT_REFERENCE_SCHEMA, BLUEPRINT_VERSION, compileBlueprintReference, validateBlueprintReference} from "./blueprint-release-governance.mjs"; export const ISSUE_REGISTRAR_SCHEMA = "agentos.issue_registrar.v1";
...
const BLUEPRINT_EMBEDDED_FIELDS = Object.freeze(["blueprint", "blueprint_content", "blueprintContent", "advice", "advisory", "advisory_inputs", "advisoryInputs", "batching_suggestions", "batchingSuggestions", "implementation_suggestions", "implementationSuggestions", "producer_notice", "producerNotice", "acknowledgement_required", "acknowledgementRequired"]);
function rejectEmbeddedBlueprintFields(value) { if (!isRecord(value)) return; for (const field of BLUEPRINT_EMBEDDED_FIELDS) assert(value[field] === undefined || value[field] === null, "ISSUE_REGISTRAR_BLUEPRINT_EMBEDDED_FORBIDDEN", `Blueprint ${field} must not be embedded in canonical issue state`); }
export function normalizeBlueprintReference(value) { if (value === undefined || value === null) return null; assert(isRecord(value), "ISSUE_REGISTRAR_BLUEPRINT_REFERENCE_INVALID", "Blueprint reference must be an object"); rejectEmbeddedBlueprintFields(value); if (value.schema !== undefined) assert(value.schema === BLUEPRINT_REFERENCE_SCHEMA, "ISSUE_REGISTRAR_BLUEPRINT_REFERENCE_INVALID", "Blueprint reference schema is invalid"); if (value.version !== undefined) assert(value.version === BLUEPRINT_VERSION, "ISSUE_REGISTRAR_BLUEPRINT_REFERENCE_INVALID", "Blueprint reference version is invalid"); const reference = value.schema === BLUEPRINT_REFERENCE_SCHEMA ? clone(value) : compileBlueprintReference({releaseId: value.release_id ?? value.releaseId, path: value.path ?? value.path_ref ?? value.release_path, sha256: value.sha256 ?? value.release_sha256 ?? value.releaseSha256}); validateBlueprintReference(reference); return reference; }
function normalizeBlueprintSubmission(value) { rejectEmbeddedBlueprintFields(value); return normalizeBlueprintReference(value?.blueprint_reference ?? value?.blueprintReference ?? null); }
...
function buildSubmissionRecord(input, {prefix, year, number, nowUtc, reservationId, historical = false} = {}) {
  ...
  const complete = missing.length === 0 && invalid.length === 0; const status = complete ? "READY" : "INTAKE_FAILED"; const lifecycleStage = complete ? "READY" : "NOT_AUTHORIZED"; const issue = {
    ...
    scope_amendment: scopeAmendment, blueprint_reference: normalizeBlueprintSubmission(input),
    evidence,
...
function patchIssue(issue, patch, nowUtc, event, actor = ISSUE_REGISTRAR_ROLE_ID) {
  ...
  if (patch.blueprint_reference !== undefined || patch.blueprintReference !== undefined) next.blueprint_reference = normalizeBlueprintReference(patch.blueprint_reference ?? patch.blueprintReference); rejectEmbeddedBlueprintFields(patch);
...
function issueDetailLines(issue, {cleared = false} = {}) {
  ...
  const lines = [
    ...
    `- Evidence: ${issue.evidence.map((entry) => `${entry.evidence_id} (${entry.reference})`).join(", ") || "(none)"}`, `- Blueprint release reference: ${issue.blueprint_reference === null || issue.blueprint_reference === undefined ? "(none)" : `${issue.blueprint_reference.release_id} (${issue.blueprint_reference.path}; ${issue.blueprint_reference.sha256})`}`,
...
export const ISSUE_REGISTRAR_BLUEPRINT_REFERENCE_SCHEMA = BLUEPRINT_REFERENCE_SCHEMA; export const ISSUE_REGISTRAR_BLUEPRINT_REFERENCE_VERSION = BLUEPRINT_VERSION; export const validateIssueBlueprintReference = normalizeBlueprintReference; export const compileIssueBlueprintReference = (value) => normalizeBlueprintReference(value);
