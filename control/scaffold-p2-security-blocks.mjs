...
const routerSpecs = [
  {slug: "owasp-web-top10-router", blockId: "specialist.security.owasp-web-top10-router", genericIds: ["SEC.OWASP_WEB_TOP10"], family: "security", title: "OWASP Web Top 10 Router", purpose: "Classify OWASP Top 10:2025 web-risk signals and select the smallest category specialist set without performing category analysis.", signals: ["SEC.OWASP_WEB_TOP10", "OWASP Top 10", "web application security", "A01:2025", "A05:2025"], context: ["web.application", "standard.edition", "applicability"], dependencies: ["specialist.security.router"]},
  {slug: "owasp-api-top10-router", blockId: "specialist.security.owasp-api-top10-router", genericIds: ["SEC.OWASP_API_TOP10"], family: "security", title: "OWASP API Security Top 10 Router", purpose: "Classify OWASP API Security Top 10 2023 signals and select the smallest category specialist set without performing category analysis.", signals: ["SEC.OWASP_API_TOP10", "OWASP API Security Top 10", "API security", "API1:2023", "API7:2023"], context: ["api.scope", "standard.edition", "applicability"], dependencies: ["specialist.security.router"]},
  {slug: "owasp-asvs-router", blockId: "specialist.security.owasp-asvs-router", genericIds: ["SEC.OWASP_ASVS"], family: "security", title: "OWASP ASVS Applicability Router", purpose: "Classify ASVS applicability and verification-level signals and route to the reusable exact edition without performing verification work.", signals: ["SEC.OWASP_ASVS", "ASVS", "application verification", "verification level"], context: ["application.scope", "standard.edition", "applicability"], dependencies: ["specialist.security.router"]},
  {slug: "access-control-router", blockId: "specialist.security.access-control-router", genericIds: ["SEC.TENANT_ACCESS_CONTROL"], family: "security", title: "Access-Control Composition Router", purpose: "Classify access-control model and boundary signals and assemble distinct authorization specialists without making authorization decisions.", signals: ["SEC.TENANT_ACCESS_CONTROL", "access control", "RBAC", "tenant isolation", "object authorization"], context: ["identity.model", "authorization.boundary", "tenant.scope"], dependencies: ["specialist.security.router"]},
  {slug: "supply-chain-router", blockId: "specialist.security.supply-chain-router", genericIds: ["SEC.SECRETS_CRYPTO_SUPPLY_CHAIN"], family: "security", title: "Secrets, Cryptography, and Supply-Chain Router", purpose: "Classify source, dependency, provenance, and cryptographic-control signals and select narrow specialists without handling secrets or certifying supply-chain posture.", signals: ["SEC.SECRETS_CRYPTO_SUPPLY_CHAIN", "SBOM", "CVE", "provenance", "supply chain"], context: ["artifact.scope", "dependency.scope", "source.authority"], dependencies: ["specialist.security.router"]},
  {slug: "data-lifecycle-router", blockId: "specialist.privacy.data-lifecycle-router", genericIds: ["PRIVACY.DATA_LIFECYCLE"], family: "data", title: "Privacy Data-Lifecycle Router", purpose: "Classify data-lifecycle signals and route to a jurisdiction-bound privacy specialist without asserting legal applicability or handling protected data.", signals: ["PRIVACY.DATA_LIFECYCLE", "data lifecycle", "retention", "erasure", "privacy"], context: ["data.class", "jurisdiction", "entity", "activity"], dependencies: []},
];
